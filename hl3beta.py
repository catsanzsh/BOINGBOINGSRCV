...
class EnhancedStation(Entity):
    ...
    def generate_uvs(self, width, height):
        """Generate UVs for the tessellated plane (simple tiling)."""
        uvs = []
        for z in range(height + 1):
            for x in range(width + 1):
                uvs.append(Vec2(
...
class EnhancedHUD(Entity):
    """
    A very simple HUD class that displays some information on the screen.
    """
    def __init__(self, player):
        super().__init__(parent=camera.ui)

        # Semi-transparent panel for background
        self.panel = Entity(
            parent=self,
            model='quad',
            scale=(0.3, 0.15),
            ...
            position=(-0.75, 0.45),
            z=-1  # Ensure it's rendered behind text
        )

        # Basic text element to show some info—e.g., health, position
        self.label = Text(
            parent=self.panel,
            text='Health: 100',
            position=(-0.14, 0.0),
    ...
    def update(self):
        """
        Called every frame; can be used to update the HUD content dynamically.
        """
        # Example: Display the player's current position (rounded)
        # You can show anything you want here (health, ammo, etc.)
        self.label.text = f'Position: {self.player.position.round(1)}'
...
class EnhancedGameManager:
    def __init__(self):
        ...
        # Create the HUD
        self.hud = EnhancedHUD(self.player)

        # Hook the update method
        self.app.run = self.run  # Override Ursina's run method so we can add our own update
    ...
    def update(self):
        """
        Called once per frame by Ursina.
        """
        self.lighting.update_exposure(time.dt)
        self.hud.update()  # Update the HUD elements

    def run(self):
        """
        Start the main game loop, hooking into Ursina's update cycle.
        """
        @self.app.event
        def update():
            self.update()
